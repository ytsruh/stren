...
- **The web app is a limited companion surface**: read-only dashboard, per-exercise history/chart pages, profile settings, weight ZIP export, admin (exercises + users + feedback inbox), and auth. Do not add web UI for creating or editing workout data.
...
- Weight reminders run on an hourly cron defined by `weightReminderCronSpec` in `cmd/main.go`. The per-user schedule lives in the users table (edited on the profile page); the cron is the only trigger — there is no admin UI to fire reminders manually.
- The weight data export is a streaming zip download produced at `GET /weight/export`, linked from the profile page ("Weight export"). The zip is built by `internal/export` (zip + CSV via Go stdlib, no new deps) and pulled from R2 via `utils.GetObject`. Photos that can't be fetched are skipped, with their keys listed in the zip's `manifest.json` `missing_photos` array.
